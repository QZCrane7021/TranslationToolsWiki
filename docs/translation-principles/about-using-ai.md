...
如果想保证翻译的质量有至少中上的水平，那么 AI 暂时不能全权负责任何一个环节。[流媒体翻译的难点][stream-media-trans] 相当特殊，即使翻译过传统的文学作品，也未必能立刻上手。但同时，不少同行极端排斥 AI。本百科对 AI 的态度是：**AI 可以辅助译者提升工作效率，但译者必须亲身参与翻译，以弥补 AI 的缺陷。**

[stream-media-trans]:./streaming-media-translation.md#streaming-media-translation-difficulties

## 字幕识别及打轴

字幕识别可以由 AI 初步完成，这样能省去大量工作量，但即使是最通用、综合性能最好的 Whisper，也无法全权负责。

首先，Whisper 无法确保下一句的一部分不被划分到上一句去。换言之，上面这段话，类比成 Whisper 识别英文并直接输出的场景，可能会是这样：

```
首先，Whisper 无法确保下一句
的一部分不被划分到上一句去。换言之，
上面这段话，类比成 Whisper 识别英文
并直接输出的场景，可能会是这样：
```

这样的断句明显不符合语言习惯。

其次，就算没有发生上述错误，Whisper 的断句也无法承接后续的中文翻译。英文中长句很多，而长句拆分后，因为语序习惯差异，中文每一小句的语义可能需要重排，每秒字数也可能过高，需要反复斟酌，甚至译者都开始翻译中文字幕了，还可能回过头去调整英文字幕的分句。更何况，它有概率把整个长句**原封不动直接输出**，严重违背字幕长度的一般限制，导致译者还要额外手动分句。
...
而显然，不同人的 “口语” 习惯也并不一致，结果 “口语化” 输出的效果非常不稳定。书面用语和口头用语并非完全对立，但 AI 无法理解这一模糊关系。时至今日，用户让 Grok 模仿贴吧用户的风格发言，Grok 还是只会满嘴 “卧槽”，仿其形而失其魂。
...
然而用 AI 生成文本时，“口语化” 这个提示词反而最有效，换作其他提示词，生成结果只会更差。如果提示 AI “书面化”，那生成的文字可能染上严重的 “翻译腔”；如果提示它具体人物的发言风格，要么效果不尽人意，反而不如完全不给提示词，要么可能涉嫌侵犯名誉权、违反其他规则而失败。
...
当然，单论这一个例子，AI 完全能正确回答。但是，Minecraft 玩家众多，而 “掘地求生” 早在 2009 年 Minecraft Classic 测试版期间就问世了，如果新造词极新，领域极小众，AI 则很可能答错。因此一定要查资料，而如果查资料用 AI 辅助，则一定要点进提供的参考链接查证，因为 AI 可能提供完全不存在或者早已失效的链接。
