...
首先，如果你是 macOS 用户，则 Aegisub 几乎无法使用。最新的 3.4.2 版本中，中文输入法存在 BUG，候选框会强制显示在全屏的左下角，输入框中也没有显示为英文的候选字；如果回退到 3.2.2 版本，触控板在音频编辑界面的灵敏度又过高，偏偏 Mac 用户很依赖触控板。

那假如你是 Windows 用户，又能否通行无忧？也不一定。虽然 Aegisub 的 Windows 发布版本标明为 x64，但实际应用是 32 位，打开后字体极为模糊；而如果你采用应用程序高 DPI 兼容，又不可避免会导致涉及图标的 UI 变得极小无比，屏幕尺寸小于 16 寸就几乎不可用。

Linux 系统下，3.4.2 需要 [自行编译源码][build-from-source]，3.2.2 可以通过 `sudo apt install aegisub` 安装，但二者都存在严重影响使用的 BUG，例如无法按秒修改时间戳，只能按帧。
...
![Tell Me👏 Tell Me👏鏡👏よ鏡👏](/assets/img/aegisub-strange-icon.png){width=300}

/// caption 
很难想象，这条金鱼实际功能是一个专门用来输入译文的小窗
///